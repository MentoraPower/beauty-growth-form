...
export function FeatureCard({ feature, className, ...props }: FeatureCardPorps) {
	...
	return (
		<div className={cn('relative group', className)} {...props}>
			...
			<div className="absolute -top-1 -left-1 w-2 h-2 border border-white/40 z-10 bg-white/20" />
			<div className="absolute -top-1 -right-1 w-2 h-2 border border-white/40 z-10 bg-white/20" />
			<div className="absolute -bottom-1 -left-1 w-2 h-2 border border-white/40 z-10 bg-white/20" />
			<div className="absolute -bottom-1 -right-1 w-2 h-2 border border-white/40 z-10 bg-white/20" />
			...
			<div className="relative p-6 overflow-hidden h-full bg-gradient-to-br from-[#F40000] to-[#A10000] rounded-lg">
				...
				<feature.icon className="text-white size-7 lg:size-8 relative z-10" strokeWidth={1.5} aria-hidden />
			<h3 className="mt-8 text-lg lg:text-xl xl:text-2xl font-bold text-white relative z-10">{feature.title}</h3>
			<p className="text-white/90 relative z-20 mt-3 text-sm lg:text-base font-light leading-relaxed">{feature.description}</p>
			</div>
		</div>
	);
}
