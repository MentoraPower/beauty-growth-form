...
export function FeatureCard({ feature, className, ...props }: FeatureCardPorps) {
	...
	return (
		<div className={cn('relative group', className)} {...props}>
			...
			<h3 className="mt-10 text-lg md:text-xl lg:text-2xl font-medium relative z-10">{feature.title}</h3>
			<p className="text-muted-foreground relative z-20 mt-2 text-base md:text-lg font-light leading-relaxed">{feature.description}</p>
			</div>
		</div>
	);
}
