...
function StaticLeadRow({ lead, isSelected, onLeadClick, onToggleSelection, tags = [] }: Omit<SortableLeadRowProps, 'isOverlay'>) {
  ...
  return (
    <div
      ...
      className={cn(
        "grid gap-6 py-2.5 px-3 hover:bg-muted/40 rounded cursor-pointer transition-colors group border-b border-white/[0.04] dark:border-white/[0.04] last:border-b-0",
        "grid-cols-[40px_minmax(140px,1.5fr)_minmax(160px,1fr)_minmax(120px,1fr)_minmax(120px,1fr)_minmax(100px,1fr)_minmax(80px,0.5fr)_40px]",
        isSelected && "bg-primary/5"
      )}
    >
      ...
      <div className="p-4">
        ...
        <div className="space-y-1">
          {pipelines.map((pipeline) => {
            ...
            return (
              <div key={pipeline.id} className="bg-card rounded-lg border border-border mb-3 last:mb-0">
                {/* Pipeline Header */}
